...
def update_array():
    ...
    canvas = Canvas(sorting_window, bg="grey", width=root.winfo_screenwidth(), height=root.winfo_screenheight())
    canvas.pack()
    ...
    try:
        speed_val = ent2.get().strip()
        speed = int(speed_val) if speed_val else 100
    except ValueError:
        speed = 100  # alapértelmezett sebesség ms
...
def alsovonal(arr, canvas):
    if len(arr) == 0:
        return  # üres lista ellenőrzés
    bar_width = (root.winfo_screenwidth() - 50) / len(arr)
    canvas_height = root.winfo_screenheight()
    for i in range(len(arr)):
        tavy = (50 if (i % 2 == 0 or i == 0) else 20) if len(arr) > 150 else 20
        canvas.create_text(20 + i * bar_width, (canvas_height / 2) - 100 + tavy, text=str(i), tags="Szamok", fill="lightgreen", angle=90)
...
def rajzold_ki(arr, y, canvas, selected=[]):
    canvas_height = root.winfo_screenheight()
    max_height = max(arr) if arr and max(arr) > 0 else 1  # zero/üres ellenőrzés
    bar_width = (root.winfo_screenwidth() - 50) / len(arr)
    canvas.delete("palcak")

    for i in range(len(arr)):
        bar_height = (arr[i] / max_height) * canvas_height / 3
